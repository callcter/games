...
try {
  ...
  // v5-B/C 起数独按钮位置随 layout 移动：浏览器内复算当前布局取真实坐标。
  const sdLayout = async () => evaluate('(async()=>{const L=await import("/src/games/sudoku/layout.ts");const P=await import("/src/games/puzzle-kit/play-area.ts");return L.sudokuLayout(P.measurePlayArea(768,__scene.scale.height,{bottom:78,horizontalPadding:36}),__scene.state.size)})()')
  ...
  for(const [width,height] of [[768,1024],[1024,768]]){
    ...
    await open('sudoku'); await resumeOrFresh()
    const sdPre = await sdLayout()
    await click(568,sdPre.modeY)
    await pause(300)
    // 切换 9×9 后布局随 size 变化（棋盘/键盘/底行全部重排），必须重取。
    const sdL = await sdLayout()
    const cell = await evaluate(`(()=>{const i=__scene.state.puzzle.indexOf(0);return {i}})()`)
    const cellPt = {x: sdL.boardLeft + (cell.i % 9 + .5) * sdL.cell, y: sdL.boardTop + (Math.floor(cell.i / 9) + .5) * sdL.cell}
    await click(cellPt.x,cellPt.y);await click(90,sdL.footerY);await click(384+(1-5)*sdL.digitStep,sdL.digitY)
    assert.equal(await evaluate(`__scene.state.notes[${cell.i}][0]`),1)
    await click(382,sdL.footerY)
    assert.equal(await evaluate(`__scene.state.notes[${cell.i}].length`),0)
    await click(384+(1-5)*sdL.digitStep,sdL.digitY)
    ...
    const ngL = () => evaluate('(async()=>{const L=await import("/src/games/nonogram/layout.ts");const G=await import("/src/games/nonogram/core/game.ts");const P=await import("/src/games/puzzle-kit/play-area.ts");const s=window.__scene;return L.nonogramLayout(P.measurePlayArea(768,s.scale.height,{bottom:78,horizontalPadding:28}),G.patternSize(s.state.level))})()')
    for(let i=0;i<9 && await evaluate('__scene.state.level!==5');i++){const l=await ngL();await click(608,l.footerY)}
    assert.equal(await evaluate('__scene.state.level'),5)
    {const l=await ngL();await click(384,l.footerY)}
    const l5=await ngL()
    const a=await point(l5.boardLeft+0.5*l5.cell,l5.boardTop+0.5*l5.cell),b=await point(l5.boardLeft+4.5*l5.cell,l5.boardTop+0.5*l5.cell)
    ...
    {const lr=await ngL();await click(160,lr.footerY)};assert.ok(await evaluate('__scene.state.marks.every(v=>v===0)'))
    await home()
    await open('maze')
    const route=await evaluate("(async()=>{const {path}=await import('/src/games/maze/core/game.ts');return path(__scene.state).slice(0,9)})()")
    const mzL = await evaluate(`(async()=>{const L=await import('/src/games/maze/layout.ts');const P=await import('/src/games/puzzle-kit/play-area.ts');const s=window.__scene;return L.mazeLayout(P.measurePlayArea(768,s.scale.height,{bottom:78,horizontalPadding:42}),s.state.size)})()`)
    const mzSize=await evaluate('__scene.state.size')
    const mazePoint=async index=>point(mzL.boardLeft+(index%mzSize+.5)*mzL.cell,mzL.boardTop+(Math.floor(index/mzSize)+.5)*mzL.cell)
    ...
    await open('tangram')
    const tgL = await evaluate(`(async()=>{const L=await import('/src/games/tangram/layout.ts');const P=await import('/src/games/puzzle-kit/play-area.ts');const s=window.__scene;return L.tangramLayout(P.measurePlayArea(768,s.scale.height,{bottom:78,horizontalPadding:40}))})()`)
    await click(135,tgL.actionY)
    for(let i=0;i<4;i++)await click(608,tgL.footerY)
    assert.equal(await evaluate('__scene.state.level'),4)
    await screenshot(`tangram-${width}`)
    for(let i=0;i<7;i++)await click(656,tgL.actionY)
  ...
  await open('sudoku');await resumeOrFresh();await click(568,await evaluate('(async()=>{const {sudokuLayout}=await import("/src/games/sudoku/layout.ts");const {measurePlayArea}=await import("/src/games/puzzle-kit/play-area.ts");return sudokuLayout(measurePlayArea(768,__scene.scale.height,{bottom:78,horizontalPadding:36}),__scene.state.size).modeY})()'))
  ...
  await open('sudoku')
  await click(260,480)
  const sdL2 = await evaluate('(async()=>{const L=await import("/src/games/sudoku/layout.ts");const P=await import("/src/games/puzzle-kit/play-area.ts");return L.sudokuLayout(P.measurePlayArea(768,__scene.scale.height,{bottom:78,horizontalPadding:36}),__scene.state.size)})()')
  const blankIdx = await evaluate('__scene.state.puzzle.indexOf(0)')
  const blank = {x: sdL2.boardLeft + (blankIdx % 9 + .5) * sdL2.cell, y: sdL2.boardTop + (Math.floor(blankIdx / 9) + .5) * sdL2.cell}
  await click(blank.x,blank.y);await click(90,sdL2.footerY);await click(384+(1-5)*sdL2.digitStep,sdL2.digitY)
  assert.equal(await evaluate(`__scene.state.notes[${blankIdx}][0]`),1)
