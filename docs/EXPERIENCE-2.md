...
> `当前等级` 是本方案初始审计的工作假设，不是永久事实。每次真正改该游戏前重新试玩确认。

| 游戏 | 类型 | 当前等级 | 目标 | 关键改造 | 优先级 | 主要风险 |
| --- | --- | ---: | ---: | --- | --- | --- |
| 停车场 | 空间解谜 | L0 | L3 | 轴向直拖、阻挡、吸附、红车驶出 | P0 | 拖动与 core 提交不同步 |
| 合成水果 | 物理合成 | L2 | L4 | 跟手预览、merge squash/pop、连锁表现、结果 overlay | P0 | Matter 状态与视觉 tween 冲突 |
| 切水果 | 动作 | L2/L3 | L4 | Intro、HUD、Round start、结果 overlay、Replay | P0 | polish 影响输入延迟 |
| 水排序 | 排序解谜 | L0/L1 | L3 | 拿起试管、倾斜、液体流动、非法倒水反馈 | P1 | 动画不能先于规则提交造成错觉 |
| 七巧板 | 空间拼图 | L2 | L3/L4 | pickup、旋转反馈、磁吸、完成轮廓；扩内容 | P1 | 多点触控/翻面手势复杂 |
| 空当接龙 | 纸牌 | L1 | L3 | 拖牌、合法落点、整列跟手、自动回收反馈 | P1 | 复杂合法移动与 drag preview |
| 纸牌 Klondike | 纸牌 | L1 | L3 | 拖牌/翻牌、吸附、自动收牌、结果 flow | P1 | 横屏密度和触控目标 |
| 蜘蛛纸牌 | 纸牌 | L1 | L3 | 成组拖动、落点反馈、收组动画 | P1 | 多牌组视觉层级 |
| 2048 | 数字滑块 | L1/L2 | L3 | 预滑、统一移动 tween、merge pop、分数浮字 | P1 | 输入期间重复 swipe |
...
| 记忆翻牌 | 记忆 | L1 | L3 | 翻牌 3D 感、配对 pop、回合反馈 | P2 | 动画锁输入时长 |
| 扫雷 | 逻辑 | L1 | L2/L3 | 翻格、插旗 tactile、连锁展开、踩雷层级 | P2 | 触控插旗手势冲突 |
| 数织 | 逻辑绘图 | L1 | L3/L4 | 连划 ink 感、完成行列、整图 reveal；扩内容 | P2 | 连续输入撤销粒度 |
| 推箱子 | 格子解谜 | L1 | L3/L4 | 推箱位移、落点、错误推挤反馈；扩内容 | P2 | 保持步数/撤销精确 |
| 俄罗斯方块 | 动作/拼块 | L2 | L3 | 锁定、消行、等级变化、game-over flow | P2 | 不能增加视觉延迟影响节奏 |
| 点泡泡 | 动作 | L2 | L3 | 泡泡 burst、命中层级、combo、结束 overlay | P2 | 高频对象/声音上限 |
| 红包雨 | 动作 | L2 | L3 | 点击反馈、连击、炮仗错误反馈、结果 overlay | P2 | 奖励表现避免过度刺激 |
| 打地鼠 | 动作 | L2 | L3 | 洞口 anticipation、命中 squash、宝宝保护反馈 | P2 | 点击判定与动画状态 |
| 五子棋 | 棋类 | L1 | L3 | 落子触感、最后一步、连线、AI 思考反馈 | P2 | AI 等待不能显得卡死 |
