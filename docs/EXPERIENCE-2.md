...
## 2026-09-17 — v0.8（二轮验收退回修复：R1~R3）

- R1 纸牌轻点移动：三款 beginCardDrag 不再在按下时清空 selection（此前轻点目标牌只改选不移动），改为 moveCardDrag 首次真实位移时才切换会话——「选源牌→点目标」点选路径恢复。CDP 实测三款轻点合法移动 moves 0→1。
- R2 数织三代编号兼容：构造已发布交错版（f352e7f..c4fb94e）到当前编号的精确双射映射（图案集合一致仅顺序不同）；草档恢复按「原身份优先、交错版映射其次、相容度过半才接受」逐候选验证，无法区分的浅进度档按原身份保守恢复且进入游戏时对原档做一次性原样备份（-backup 键）；完成标记 nonogram-N（N≥9 无歧义段）经 migrateNumberedFlags 迁移，小树里程碑由新名字继承（不重发叶、叶数不变），N 0-8 两代同义保留。CDP 实测：交错版 level=6（小房子 25 格 4 已填）正确恢复、nonogram-17→nonogram-23 迁移、里程碑继承叶数不变、备份存在。
- R3 推箱子选关末排：网格改为起点 216、排距 112，末排底边 806（画布 900 内）。
- browser-smoke：open() 增加卡片级等待（修复二轮报告记录的 reload 后随机 null.click flaky），正式回归完整跑绿。
- 测试 216→221（交错版映射双射、三代草档恢复、迁移与里程碑继承、上轮 A1~A6 用例更新）；AGENTS 第 8 节数织描述同步为「原图 0-8 身份稳定 + 变体追加」。
