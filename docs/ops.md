...
- 2026-09-20 发布 `c869751`（12 个提交，v5 A–E 五批）：外部 composition 包逐行审查后
  全部合入——A/B：8 款益智（记忆/接水管/推箱子/数织/迷宫/数独/七巧板/解绳结）
  手机真正响应式（动态逻辑高、几何游戏 core 坐标经 view 映射不动 core）；
  C：物理尺寸规范（按画布真实 CSS 换算，按钮 ≥44 CSS px、字号下限分级，仅
  手机生效）；D：四款持久视图（翻牌/转管/推箱/迷宫普通一步不再整盘重建，
  连续动画+busy 门+viewRevision 防竞态）；E：大厅信息架构（继续玩主/次卡、
  四分类卡、游戏柜默认前 8+查看全部、整理图标移入柜标题）。回归脚本坐标
  随布局复算适配；75 张三视口重拍并逐张/拼图自查零问题。验证：check 250
  测试、完整 smoke、ui/modal/shell 专项、RENDER_ONLY、生产离线全通过；
  发版后 HTTPS 200、关键产物 SHA-256 一致。已推送 GitHub，待 GPT 验收。
