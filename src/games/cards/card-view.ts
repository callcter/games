...
export function createCardView(
  ...
  // Callers position cards by their top-left corner. Phaser centers a
  // Container's input origin, so keep both the artwork and hit area centered
  // around the container to make the whole visible card tappable.
  const container = scene.add.container(x + width / 2, y + height / 2)
  const graphics = new Phaser.GameObjects.Graphics(scene)
  graphics.fillStyle(faceUp ? 0xfffdf6 : 0x4770a7, 1)
  graphics.fillRoundedRect(-width / 2, -height / 2, width, height, width * 0.09)
  graphics.lineStyle(options.selected ? 5 : 2, options.selected ? 0xf0bf4f : 0x173f35, options.selected ? 1 : 0.28)
  graphics.strokeRoundedRect(-width / 2, -height / 2, width, height, width * 0.09)
  container.add(graphics)

  if (faceUp) {
    const color = card.color === 'red' ? '#c9443d' : '#173f35'
    const corner = new Phaser.GameObjects.Text(scene, -width * 0.4, -height * 0.44, `${rankLabel(card.rank)}${suitSymbol(card.suit)}`, {
      color, fontFamily: 'Georgia, Times New Roman, serif', fontSize: `${Math.max(15, width * 0.2)}px`, fontStyle: 'bold'
    })
    const suit = new Phaser.GameObjects.Text(scene, 0, height * 0.05, suitSymbol(card.suit), {
  ...
  if (options.onSelect) container.on('pointerdown', options.onSelect)
  return container
}
