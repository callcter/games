...
export class NonogramScene extends PuzzleScene {
  ...
  private draw(): void {
    ...
    const cell = size === 5 ? 84 : 44
    const boardLeft = size === 5 ? 225 : 196, boardTop = size === 5 ? 310 : 272
    this.resetView(`第 ${level + 1} / ${PATTERNS.length} 幅 · 数字表示连续填色格数`)
    this.text(384, 157, size === 5 ? '例如 2 1：填两格，至少空一格，再填一格' : '10×10 挑战 · 先从数字多的行列下手', 18, this.content)
    for (let i = 0; i < size; i++) {
      ...
      this.text(boardLeft + i * cell + cell / 2, boardTop - (size === 5 ? 70 : 62), clues(Array.from({ length: size }, (_, y) => target[y * size + i]!)).join('\n'), size === 5 ? 24 : 15, this.content)
    }
